...
function parse (options) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (text) {
      ...
      readFileToArr(filePath, cb).then(done)
    }
  })
}
...
/**
 * Handles each line and mutates data obj2ect (bound to `this`) as needed.
 *
...
function handleLine (options, line) {
  ...
  if (this.current) {
    ...
    if (listitem.exec(line)) {
      const log = line //  options.removeMarkdown ? removeMarkdown(line) : line
